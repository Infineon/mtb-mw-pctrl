...
void AcRmsPllReset_3phdd(ac_rms_pll_context_t* const context)
{
    ...
    /* Resets the DDSRF-specific variables */
    context->vars.dd_pos = 0.0F;
    context->vars.dq_pos = 0.0F;
    context->vars.dd_neg = 0.0F;
    context->vars.dq_neg = 0.0F;
    context->vars.d_pos = 0.0F;
    context->vars.d_neg = 0.0F;
    context->vars.q_pos = 0.0F;
    context->vars.q_neg = 0.0F;

    /* Resets the DDSRF filters with init value from configuration */
    context->vars.dd_pos_filt = context->config.lpc_dd_init;
    context->vars.dq_pos_filt = context->config.lpc_dd_init;
...
void AcRmsPllInit_3phdd(ac_rms_pll_context_t* const context)
{
    ...
    /* Initializes the DDSRF sequence filter states */
    context->vars.dd_pos_filt = context->config.lpc_dd_init;
    context->vars.dq_pos_filt = context->config.lpc_dd_init;
    context->vars.dd_neg_filt = context->config.lpc_dd_init;
    context->vars.dq_neg_filt = context->config.lpc_dd_init;
    ...
    /* Resets the other DDSRF variables */
    AcRmsPllReset_3phdd(context);
}
 ...
 *
 * This function calls the inline implementation to provide a non-inline version
 *
 * @param[in] context   : Pointer to the context structure
                             ...
                             float32_t* rms)
{
    ...
    AcRmsPll_3phdd_inline(context, alpha, beta, rms);
}
